...
using Environment = QnaMakerApi.Requests.Environment;

namespace QnaMakerApi {
	public class QnaMakerClient : IDisposable {
		private const string BaseAddress = "https://westus.api.cognitive.microsoft.com/qnamaker/v4.0/knowledgebases/";
		private const int MaxQnaPairs = 1000;
		private const int MaxQnaUrls = 5;

		private readonly HttpClient _client;

		public QnaMakerClient(string subscriptionKey, string baseAddress = BaseAddress) {
		...
		/// </returns>
		public async Task<DownloadKnowledgeBaseResponse> DownloadKnowledgeBase(Guid id, Environment environment) {
		...
		public async Task<DownloadKnowledgeBaseResponse> DownloadKnowledgeBase(DownloadKnowledgeBaseRequest req) {
			var response = await Send(HttpMethod.Get, $"{req.KnowledgeBaseId}/{req.Environment.ToString()}/qna");
			var result = DownloadKnowledgeBaseResponse.FromJson(response);
			result.BlobUrl = response.Replace("\"", "");
			return result;
		}
